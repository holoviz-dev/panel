...
class panel.io.server.AutoloadJsHandler(tornado_app: BokehTornado, \*args: Any, **kw: Any)
...
class panel.io.server.DocHandler(tornado_app: BokehTornado, \*args: Any, **kw: Any)
Bases:
...
A synchronous call made from the server’s own running event loop cannot
block that loop. In that case cleanup is scheduled and this method
returns; await `wait_until_stopped()` for the
completion barrier. Async callers may instead use
`stop_async()` directly.

Returns:
None

class panel.io.server.WSHandler(tornado_app: Application, \*args: Any, **kw: Any)
Bases: `WSHandler`
